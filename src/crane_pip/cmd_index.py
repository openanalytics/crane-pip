...
register_parser.add_argument(
    "url", help="index-url of the private package index that is protected by a crane server."
)
...
def entrypoint_register(args) -> int:
    ...
    server_configs[args.url] = ServerConfig(
        client_id=ns["client-id"], token_url=ns["token-url"], device_url=ns["device-url"]
    )
...
list_parser.add_argument(
    "--url", "-u", nargs="*", help="Set of urls to for which only the settings should be returned."
)
...
remove_parser.add_argument(
    "url", help="Index url to remove from the registered list of crane indexes."
)
...
def entrypoint_remove(args) -> int:
    try:
        ...
        close_match = get_close_matches(word=args.url, possibilities=known_indexes, n=1)
        if close_match:

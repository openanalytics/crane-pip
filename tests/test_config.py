...
def test_existing_server_configs(tmp_server_configs_prefilled: ServerConfigs, configs):
    ...
    assert "url1" in server_configs and "url2" in server_configs, "Existing server_configs is read correctly"
    assert server_configs.get("url1") == configs[0]
    assert server_configs["url2"] == configs[1]
    assert server_configs.get('url3') == None, "get operation works as expected on server_configs object"
